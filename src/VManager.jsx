...
function checkSubscription(pr) {
  ...
  // Admin manually set paid=false → blocked
  ...
  // No expiry set → treat as active (legacy or error)
  if (!pr.expires_at) return { active: true, expired: false, daysLeft: 999, status: "active" };
...
export default function App() {
  ...
  const handleLogin = useCallback(async (supaUser, pr) => {
    ...
    // Always re-fetch fresh profile from Supabase for accurate subscription status
    let freshPr = pr;
    try { const fp = await sbGetProfile(actualUser.id); if (fp) freshPr = fp; } catch {}

    const sub = checkSubscription(freshPr);
    setSubStatus(sub.status);

    if (!sub.active) {
      // Blocked or expired — don't let in, sign out
      await getSB().auth.signOut();
      return;
    }
