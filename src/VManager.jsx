  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        <div className="divider" />
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}><span style={{ color: T.textMid }}>Plan</span><span className="badge badge-amber">{freshProfile?.plan || userProfile?.plan || "Free"}</span></div>
        <div className="divider" />
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}><span style={{ color: T.textMid }}>Member Since</span><span style={{ fontWeight: 600, fontSize: 12, color: T.textMid }}>{(freshProfile?.paid_at || userProfile?.paid_at) ? new Date(freshProfile?.paid_at || userProfile?.paid_at).toLocaleDateString("en-IN") : "—"}</span></div>
      </div>
  ...
  const handlePayment = async () => {
    ...
    const createAccount = async (paymentId) => {
      ...
      try {
        ...
        await sbSaveProfile(supaUser.id, profilePayload);
        setScreen("success");
        setTimeout(() => onLogin(supaUser, profilePayload), 1800);
      } catch (e) {
...
export default function App() {
  ...
  const handleLogin = useCallback(async (supaUser, pr) => {
    ...
    setUser(supaUser);
    // Set immediately from passed profile (works for new registrations)
    const localPr = pr || {};
    setUserProfile(localPr);
    if (localPr.name) setProfile(prev => ({ ...prev, name: localPr.name, phone: localPr.phone || "" }));
    // Then re-fetch from Supabase to get confirmed data (small delay for write propagation)
    setTimeout(async () => {
      try {
        const fresh = await sbGetProfile(supaUser.id);
        if (fresh) {
          setUserProfile(fresh);
          if (fresh.name) setProfile(prev => ({ ...prev, name: fresh.name, phone: fresh.phone || "" }));
        }
      } catch {}
    }, 1000);
