...
// ─── PROFILE ─────────────────────────────────────────────────────────────────
function Profile({ showToast, profile, setProfile, onLogout, userProfile }) {
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        ...
        {[["💾", "Backup Data", "Export all data to cloud"], ["📤", "Export Data", "Download as CSV/Excel"], ["📥", "Restore Data", "Import from backup"]].map(([ic, t, d]) => (
          <div key={t} style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }} onClick={() => showToast(t + " initiated...")}>
            <div style={{ width: 40, height: 40, background: T.bg, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{ic}</div>
            <div style={{ flex: 1 }}>
      ...
      <div className="card">
        ...
        <div className="divider" />
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}><span style={{ color: T.textMid }}>Plan</span><span className="badge badge-amber">{userProfile?.plan || "Premium"}</span></div>
        <div className="divider" />
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}><span style={{ color: T.textMid }}>Payment ID</span><span style={{ fontWeight: 600, fontSize: 11, color: T.textLight }}>{userProfile?.payment_id || "—"}</span></div>
      </div>

      {/* Subscription & Logout */}
      {onLogout && (
        <div style={{ marginTop: 12 }}>
          <div style={{ background: T.amberLight, borderRadius: 14, padding: "12px 16px", marginBottom: 10, display: "flex", gap: 10, alignItems: "center" }}>
            <span style={{ fontSize: 20 }}>💡</span>
            <div style={{ fontSize: 12, color: T.amberDark, lineHeight: 1.5 }}>
              Save your <b>Payment ID</b> above — you'll need it to restore access on a new device.
            </div>
          </div>
...
// ─── RAZORPAY CONFIG ──────────────────────────────────────────────────────────
// ⚠️ Replace with your Razorpay Key ID from https://dashboard.razorpay.com
// Settings → API Keys → Generate Key (starts with rzp_test_... or rzp_live_...)
...
// ─── AUTH SCREEN (Login + Register + Payment) ─────────────────────────────────
function AuthScreen({ onLogin }) {
  const [screen, setScreen] = useState("login"); // login | register | plan | paying | success
  // Login fields
  ...
  useEffect(() => {
    getSB().auth.getSession().then(({ data: { session } }) => {
      ...
      sbGetProfile(session.user.id).then(pr => {
        if (pr?.paid) onLogin(session.user, pr);
      });
    });
  }, []);
  ...
  const mapErr = (msg) => {
    ...
    if (msg.includes("already registered") || msg.includes("User already registered")) return "Email already registered — Sign In instead.";
    if (msg.includes("Password should") || msg.includes("password")) return "Password must be at least 6 characters.";
    ...
    if (msg.includes("Email rate limit")) return "Too many attempts. Please wait a minute and try again.";
    return msg;
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!pr?.paid) {
        // Registered but never paid — take them to payment
        setRName(pr?.name || ""); setRPhone(pr?.phone || ""); setREmail(lEmail.trim());
        setLoading(false);
        setErr(""); setInfo("Your account exists but payment is pending. Please complete payment.");
        setScreen("plan");
        return;
      }
  ...
  // ── REGISTER (step 1 — collect info) ──
  const handleRegister = () => {
    setErr("");
    if (!rName.trim())  return setErr("Please enter your full name.");
    if (!rPhone.trim() || rPhone.trim().length < 10) return setErr("Enter a valid 10-digit phone number.");
  ...
  const handlePayment = async () => {
    ...
    const options = {
      ...
      handler: async (response) => {
        ...
        try {
          ...
          // 3. Go to app
          setScreen("success");
          setTimeout(() => onLogin(user, profilePayload), 1800);
        } catch (e) {
          setErr("Payment was received but account setup failed: " + e.message + ". Contact support with Payment ID: " + response.razorpay_payment_id);
          setScreen("plan");
          setLoading(false);
        }
      },
    ...
    try {
      ...
      setErr("Could not open payment. Check your Razorpay key.");
      setLoading(false);
    }
  };
      ...
      <div style={{ textAlign: "center", color: "white" }}>
        {screen === "success"
          ? <><div style={{ fontSize: 72, marginBottom: 16 }}>🎉</div>
              ...
              <div style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", marginTop: 8 }}>Payment successful · Opening VManager…</div></>
  ...
  if (screen === "plan") return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg, ${T.navy}, #1a2a6c)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ width: "100%", maxWidth: 390 }}>
        <button onClick={() => { setScreen(screen === "plan" && lEmail ? "login" : "register"); setErr(""); setInfo(""); }}
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 900, fontSize: 34, color: T.amber }}>V<span style={{ color: "white" }}>Manager</span></div>
          <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, marginTop: 6 }}>Hi {rName.split(" ")[0] || "there"}! Choose your plan 👇</div>
        </div>

        {info && <div style={{ background: T.amberLight, color: T.amberDark, padding: "12px 16px", borderRadius: 12, fontSize: 13, fontWeight: 600, marginBottom: 14 }}>ℹ️ {info}</div>}
        {err  && <div style={{ background: T.roseLight, color: T.rose, padding: "12px 16px", borderRadius: 12, fontSize: 13, fontWeight: 600, marginBottom: 14 }}>⚠️ {err}</div>}

        {PLANS.map(p => (
          <div key={p.id} onClick={() => setPlan(p.id)}
            style={{ background: plan === p.id ? "white" : "rgba(255,255,255,0.07)", border: plan === p.id ? `2px solid ${T.amber}` : "2px solid rgba(255,255,255,0.12)", borderRadius: 16, padding: "16px 18px", marginBottom: 12, cursor: "pointer", transition: "all 0.2s", position: "relative" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: 16, color: plan === p.id ? T.navy : "white" }}>{p.label}</div>
        ...
        <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 14, padding: "14px 16px", marginBottom: 16 }}>
          ...
          {["📦 Full Inventory Management","🛒 Sales & Invoice Generator","💬 WhatsApp Invoice Sharing","📊 Reports & Analytics","🔔 Smart Alerts","⚡ AAJ KI ENERGY Videos","☁️ Cloud Sync via Supabase"].map(f => (
            <div key={f} style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", padding: "3px 0", display: "flex", gap: 8 }}><span style={{ color: T.amber }}>✓</span><span>{f}</span></div>
          ))}
        </div>
        ...
        <button onClick={handlePayment} disabled={loading}
          ...
          {loading
            ? <><span style={{ width: 20, height: 20, border: `3px solid ${T.navy}30`, borderTop: `3px solid ${T.navy}`, borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite" }} /><span>Opening Payment…</span></>
            : <><span style={{ fontSize: 18 }}>🔒</span><span>Pay {PLANS.find(p => p.id === plan)?.display} Securely</span></>
          }
        </button>
        <div style={{ textAlign: "center", marginTop: 10, fontSize: 11, color: "rgba(255,255,255,0.35)" }}>Powered by Razorpay · UPI · Cards · Net Banking</div>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg, ${T.navy} 0%, #1a2a6c 60%, #0d3b6e 100%)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, overflowY: "auto" }}>
      ...
      <div style={{ width: "100%", maxWidth: 390, background: "white", borderRadius: 28, overflow: "hidden", boxShadow: "0 32px 80px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ padding: "0 24px 28px" }}>
          ...
          {info && <div style={{ background: T.amberLight, border: `1px solid ${T.amber}40`, color: T.amberDark, padding: "12px 14px", borderRadius: 12, fontSize: 13, fontWeight: 600, marginBottom: 16 }}>ℹ️ {info}</div>}

          {/* ── REGISTER FIELDS ── */}
          {!isLogin && (
            <>
              {[
                ["Full Name *", "👤", rName, setRName, "text", "Your full name"],
                ["Phone Number *", "📞", rPhone, setRPhone, "tel", "9876543210"],
              ].map(([lbl, ic, val, setter, type, ph]) => (
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input className="input" style={{ paddingLeft: 40 }} value={isLogin ? lEmail : rEmail} onChange={e => isLogin ? setLEmail(e.target.value) : setREmail(e.target.value)} placeholder="you@gmail.com" type="email" autoComplete="email" />
            </div>
          </div>
          ...
          <div style={{ marginBottom: isLogin ? 20 : 12 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input className="input" style={{ paddingLeft: 40, paddingRight: 44 }} value={isLogin ? lPass : rPass} onChange={e => isLogin ? setLPass(e.target.value) : setRPass(e.target.value)} placeholder={isLogin ? "Your password" : "Min 6 characters"} type={isLogin ? (lShowP ? "text" : "password") : (rShowP ? "text" : "password")} autoComplete={isLogin ? "current-password" : "new-password"}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input className="input" style={{ paddingLeft: 40, borderColor: rPass2 && rPass2 !== rPass ? T.rose : rPass2 && rPass2 === rPass ? T.emerald : T.border }}
                  value={rPass2} onChange={e => setRPass2(e.target.value)} placeholder="Re-enter password" type={rShowP ? "text" : "password"} autoComplete="new-password"
          ...
          <button
            ...
            {loading
              ...
              : isLogin ? "Sign In to VManager →" : "Continue to Payment →"
            }
          </button>
          ...
          <div style={{ textAlign: "center", marginTop: 16, fontSize: 13, color: T.textMid }}>
            ...
            <span style={{ color: T.navy, fontWeight: 700, cursor: "pointer", textDecoration: "underline" }}
              ...
              {isLogin ? "Register & Subscribe" : "Sign In"}
            </span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 16, padding: "10px 14px", background: T.bg, borderRadius: 10 }}>
            <span style={{ fontSize: 18 }}>🔒</span>
            <div style={{ fontSize: 11, color: T.textLight, lineHeight: 1.4 }}>
              {isLogin ? "Your account is protected by Supabase Auth." : "Secure payment via Razorpay · UPI · Cards · Net Banking accepted."}
            </div>
          </div>
        </div>
      </div>
    ...
    <>
      ...
      <div className="app-wrap">
        ...
        <div className="content">
          ...
          {tab === "profile"   && <Profile showToast={showToast} profile={profile} setProfile={setProfile} onLogout={handleLogout} userProfile={userProfile} />}
        </div>
