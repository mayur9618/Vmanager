      ...
      <div>
        ...
        <button
          ...
          onClick={() => {
            ...
            const distName  = profile ? profile.name  : "";
            const distPhone = profile ? profile.phone : "";
            const distId    = profile ? profile.vestId : "";
            const distAddr  = profile ? profile.address : "";
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .page { background: white; max-width: 480px; margin: 0 auto; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.12); }
  .header { background: #0B1437; padding: 24px 24px 20px; color: white; }
  .logo { font-size: 28px; font-weight: 900; letter-spacing: -1px; }
  .logo span { color: #F5A623; }
  .sub { font-size: 11px; color: rgba(255,255,255,0.5); margin-top: 2px; }
  .inv-num { margin-top: 16px; font-size: 12px; color: rgba(255,255,255,0.6); }
  .inv-num b { color: #F5A623; font-size: 15px; }
  .body { padding: 20px 24px; }
  .section-title { font-size: 10px; font-weight: 700; color: #8896A5; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 10px; margin-top: 18px; }
  .row { display: flex; justify-content: space-between; padding: 7px 0; border-bottom: 1px solid #f0f0f0; font-size: 13px; }
  .row .label { color: #8896A5; }
  .row .val { font-weight: 600; color: #0B1437; text-align: right; max-width: 60%; }
  .items-header { display: flex; justify-content: space-between; font-size: 10px; font-weight: 700; color: #8896A5; text-transform: uppercase; padding: 8px 0 6px; border-bottom: 2px solid #0B1437; }
  ...
  .item-qty { width: 40px; text-align: center; color: #8896A5; }
  .item-price { width: 80px; text-align: right; font-weight: 600; }
  .total-box { background: #0B1437; border-radius: 10px; padding: 14px 16px; margin-top: 16px; display: flex; justify-content: space-between; align-items: center; }
  .total-box .label { color: rgba(255,255,255,0.6); font-size: 12px; font-weight: 700; text-transform: uppercase; }
  .total-box .amount { color: #F5A623; font-size: 22px; font-weight: 900; }
  .status { text-align: center; padding: 10px; border-radius: 8px; font-size: 12px; font-weight: 700; margin-top: 12px; }
  .paid { background: #e6fbf5; color: #0DBF8C; }
  .pending { background: #fff3d6; color: #D4881A; }
  .footer { background: #f9f9f9; padding: 16px 24px; border-top: 1px solid #eee; }
  .footer .dist-title { font-size: 10px; color: #8896A5; text-transform: uppercase; font-weight: 700; margin-bottom: 8px; }
  .footer .dist-info { font-size: 12px; color: #333; line-height: 1.8; }
  .footer .thank { text-align: center; margin-top: 14px; font-size: 12px; color: #8896A5; }
...
<div class="page">
  <div class="header">
    <div class="logo">V<span>Manager</span></div>
    <div class="sub">Vestige Distributor Invoice</div>
    <div class="inv-num">Invoice <b>#${invNum}</b> &nbsp;·&nbsp; ${viewInvoice.date}</div>
  </div>
  <div class="body">
    <div class="section-title">Customer Details</div>
    <div class="row"><span class="label">Name</span><span class="val">${viewInvoice.customerName}</span></div>
    ${invCust && invCust.phone ? `<div class="row"><span class="label">Mobile</span><span class="val">${invCust.phone}</span></div>` : ""}
    ${invCust && invCust.address ? `<div class="row"><span class="label">Address</span><span class="val">${invCust.address}</span></div>` : ""}
    <div class="row"><span class="label">Payment</span><span class="val">${viewInvoice.paymentMethod}</span></div>

    <div class="section-title">Items Purchased</div>
    <div class="items-header"><span>Product</span><span>Qty</span><span>Amount</span></div>
    ${viewInvoice.items.map(i => `
    <div class="item-row">
      <span class="item-name">${i.name}</span>
      <span class="item-qty">${i.qty}</span>
      <span class="item-price">&#8377;${(i.price * i.qty).toLocaleString("en-IN")}</span>
    </div>`).join("")}

    <div class="total-box">
      <span class="label">Total Amount</span>
      <span class="amount">&#8377;${viewInvoice.total.toLocaleString("en-IN")}</span>
    </div>
  ...
  <div class="footer">
    <div class="dist-title">Distributor Details</div>
    <div class="dist-info">
    ...
    <div class="thank">Thank you for your purchase! 🙏<br/><span style="font-size:10px;color:#bbb">Powered by VManager</span></div>
  </div>
</div>
</body>
</html>`;

            // Open in new tab — user can Print → Save as PDF → Share on WhatsApp
            ...
            if (win) {
              ...
              showToast("📄 Invoice PDF opened — Save as PDF then share on WhatsApp");
            } else {
              ...
              showToast("📄 Invoice downloaded — open and print as PDF");
            }
          }}
        >
