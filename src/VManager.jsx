...
function Inventory({ products, setProducts, showToast }) {
  ...
  const [delConfirm, setDelConfirm] = useState(null); // product to delete
  ...
  const save = () => {
    if (!form.name.trim()) return showToast("Product name is required");
    if (editing) {
  ...
  const del = (id) => { setDelConfirm(products.find(p => p.id === id)); };
  const confirmDel = (id) => { setProducts(products.filter(p => p.id !== id)); setDelConfirm(null); showToast("Product deleted ✓"); };
  ...
  return (
    <div>
      ...
      {delConfirm && (
        <Modal open={true} onClose={() => setDelConfirm(null)} title="Delete Product?">
          <div style={{ marginBottom: 16 }}>
            <div style={{ background: T.roseLight, borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: T.rose }}>{delConfirm.name}</div>
              <div style={{ fontSize: 12, color: T.textMid, marginTop: 4 }}>
                {delConfirm.category} · Stock: {delConfirm.stock} units · DP: {fmt(delConfirm.buyPrice)}
              </div>
            </div>
            <div style={{ fontSize: 13, color: T.textMid, lineHeight: 1.6 }}>
              ⚠️ This will permanently delete this product. Sales records are kept.
            </div>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button className="btn btn-ghost" style={{ flex: 1 }} onClick={() => setDelConfirm(null)}>Cancel</button>
            <button onClick={() => confirmDel(delConfirm.id)}
              style={{ flex: 1, padding: "12px", background: T.rose, color: "white", border: "none", borderRadius: 12, fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
              🗑️ Yes, Delete
            </button>
          </div>
        </Modal>
...
// ─── SEND PDF BUTTON ─────────────────────────────────────────────────────────
function SendPdfButton({ viewInvoice, invCust, phone, profile, showToast }) {
  const [state, setState] = useState("idle"); // idle | generating | ready | sharing
  ...
  const loadLibs = () => new Promise((resolve, reject) => {
    ...
    const err  = () => reject(new Error("Library load failed"));
    if (!window.html2canvas) {
      const s = document.createElement("script");
      s.src = "https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js";
      s.onload = done; s.onerror = err; document.head.appendChild(s);
    } else done();
    if (!window.jspdf) {
      const s = document.createElement("script");
  ...
  const buildEl = () => {
    const invNum = String(viewInvoice.id).slice(-6).toUpperCase();
    const isPaid = viewInvoice.paymentStatus === "paid";
    const d = profile || {};
    const el = document.createElement("div");
    el.style.cssText = "width:420px;background:white;font-family:Arial,sans-serif;font-size:13px;position:fixed;left:-9999px;top:0;";
    el.innerHTML = `
      <div style="background:#0B1437;padding:22px 22px 18px;color:white;">
        <div style="font-size:26px;font-weight:900;letter-spacing:-1px;">V<span style="color:#F5A623">Manager</span></div>
        <div style="font-size:10px;color:rgba(255,255,255,0.5);margin-top:2px;">Vestige Distributor Invoice</div>
        <div style="margin-top:12px;font-size:11px;color:rgba(255,255,255,0.5);">Invoice <b style="color:#F5A623;font-size:14px;">#${invNum}</b> &nbsp;·&nbsp; ${viewInvoice.date}</div>
      </div>
      <div style="padding:18px 22px;">
        <div style="font-size:9px;font-weight:700;color:#8896A5;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Customer Details</div>
        <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:14px;">
          <tr><td style="color:#8896A5;padding:5px 0;border-bottom:1px solid #f0f0f0;">Name</td><td style="font-weight:600;text-align:right;padding:5px 0;border-bottom:1px solid #f0f0f0;">${viewInvoice.customerName}</td></tr>
          ${invCust?.phone ? `<tr><td style="color:#8896A5;padding:5px 0;border-bottom:1px solid #f0f0f0;">Mobile</td><td style="font-weight:600;text-align:right;padding:5px 0;border-bottom:1px solid #f0f0f0;">${invCust.phone}</td></tr>` : ""}
          ${invCust?.address ? `<tr><td style="color:#8896A5;padding:5px 0;border-bottom:1px solid #f0f0f0;">Address</td><td style="font-weight:600;text-align:right;padding:5px 0;border-bottom:1px solid #f0f0f0;">${invCust.address}</td></tr>` : ""}
          <tr><td style="color:#8896A5;padding:5px 0;">Payment</td><td style="font-weight:600;text-align:right;padding:5px 0;">${viewInvoice.paymentMethod}</td></tr>
        </table>
        <div style="font-size:9px;font-weight:700;color:#8896A5;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Items Purchased</div>
        <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:14px;">
          <thead><tr style="border-bottom:2px solid #0B1437;">
            <th style="text-align:left;padding:5px 0;font-size:9px;color:#8896A5;font-weight:700;text-transform:uppercase;">Product</th>
            <th style="text-align:center;width:40px;padding:5px 0;font-size:9px;color:#8896A5;font-weight:700;text-transform:uppercase;">Qty</th>
            <th style="text-align:right;width:80px;padding:5px 0;font-size:9px;color:#8896A5;font-weight:700;text-transform:uppercase;">Amount</th>
          </tr></thead>
          <tbody>${viewInvoice.items.map(i => `<tr style="border-bottom:1px solid #f5f5f5;">
            <td style="padding:6px 0;color:#333;">${i.name}</td>
            <td style="padding:6px 0;text-align:center;color:#8896A5;">${i.qty}</td>
            <td style="padding:6px 0;text-align:right;font-weight:600;">&#8377;${(i.price * i.qty).toLocaleString("en-IN")}</td>
          </tr>`).join("")}</tbody>
        </table>
        <div style="background:#0B1437;border-radius:10px;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
          <span style="color:rgba(255,255,255,0.6);font-size:11px;font-weight:700;text-transform:uppercase;">Total Amount</span>
          <span style="color:#F5A623;font-size:20px;font-weight:900;">&#8377;${viewInvoice.total.toLocaleString("en-IN")}</span>
        </div>
        <div style="text-align:center;padding:8px;border-radius:8px;font-size:11px;font-weight:700;background:${isPaid ? "#e6fbf5" : "#fff3d6"};color:${isPaid ? "#0DBF8C" : "#D4881A"};">
          ${isPaid ? "✓ Payment Received — " + viewInvoice.paymentMethod : "⏳ Payment Pending — Please collect"}
        </div>
      </div>
      <div style="background:#f9f9f9;padding:14px 22px;border-top:1px solid #eee;">
        <div style="font-size:9px;font-weight:700;color:#8896A5;text-transform:uppercase;margin-bottom:6px;">Distributor Details</div>
        <div style="font-size:11px;color:#333;line-height:1.9;">
          ${d.name ? "👤 " + d.name + "<br/>" : ""}
          ${d.phone ? "📞 " + d.phone + "<br/>" : ""}
          ${d.vestId ? "🆔 Vestige ID: " + d.vestId + "<br/>" : ""}
          ${d.address ? "📍 " + d.address : ""}
        </div>
        <div style="text-align:center;margin-top:12px;font-size:11px;color:#8896A5;">Thank you for your purchase! 🙏</div>
      </div>`;
    document.body.appendChild(el);
    return el;
  };

  // STEP 1: Generate PDF in background (not user-gesture-sensitive)
  const handleGenerate = async () => {
    ...
    try {
      ...
      const invNum = String(viewInvoice.id).slice(-6).toUpperCase();
      const name   = "Invoice_" + invNum + "_" + viewInvoice.customerName.replace(/\s+/g, "_") + ".pdf";
      setPdfBlob(pdf.output("blob"));
      setFileName(name);
      setState("ready");
      showToast("✅ PDF ready! Tap Share to send.");
    } catch (e) {
      setState("idle");
  ...
  // STEP 2: Share — called directly from tap (preserves user gesture)
  const handleShare = () => {
    setState("sharing");
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      navigator.share({
        files: [file],
        title: "Invoice — " + viewInvoice.customerName,
        text:  "Invoice from " + (profile?.name || "VManager"),
      }).then(() => {
        showToast("✅ PDF sent!");
        setState("idle"); setPdfBlob(null);
      }).catch(e => {
        if (e.name !== "AbortError") showToast("❌ Share failed. Downloading instead.");
      ...
      // Browser doesn't support file sharing — download directly
      const url = URL.createObjectURL(pdfBlob);
      const a = document.createElement("a"); a.href = url; a.download = fileName; a.click();
      URL.revokeObjectURL(url);
  ...
  const custFirst = invCust?.name?.split(" ")[0] || "Customer";
    ...
    <>
      ...
      {state === "idle" && (
        <button className="btn btn-full"
          style={{ marginTop: 8, padding: "16px", fontSize: 15, fontWeight: 800, background: T.navy, color: "white", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}
      ...
      {state === "generating" && (
        <div style={{ marginTop: 8, padding: "16px", background: T.navy, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 12, color: "white", fontWeight: 700, fontSize: 14, animation: "pulse 1.2s infinite" }}>
          <span style={{ width: 22, height: 22, border: "3px solid rgba(255,255,255,0.2)", borderTop: "3px solid #F5A623", borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite", flexShrink: 0 }} />
          Generating PDF… please wait
        </div>
      )}

      {/* State: ready — show Share button (direct tap = valid user gesture) */}
      {state === "ready" && (
        <div style={{ marginTop: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#e6fbf5", borderRadius: 12, padding: "10px 14px", marginBottom: 8 }}>
            <span style={{ fontSize: 18 }}>✅</span>
            <div style={{ flex: 1, fontSize: 13, color: T.emerald, fontWeight: 600 }}>PDF is ready! Tap below to send.</div>
            <button onClick={() => { setState("idle"); setPdfBlob(null); }} style={{ background: "none", border: "none", color: T.textLight, fontSize: 18, cursor: "pointer" }}>✕</button>
          </div>
          <button className="btn btn-full"
            style={{ padding: "16px", fontSize: 15, fontWeight: 800, background: "#25D366", color: "white", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, boxShadow: "0 6px 20px rgba(37,211,102,0.35)" }}
            onClick={handleShare}>
            <span style={{ fontSize: 22 }}>📤</span>
            <span>Share PDF to {phone ? custFirst + " · " + phone : custFirst}</span>
          </button>
        </div>
      )}
      ...
      {/* State: sharing */}
      {state === "sharing" && (
        <div style={{ marginTop: 8, padding: "16px", background: "#25D366", borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 12, color: "white", fontWeight: 700, fontSize: 14 }}>
          <span style={{ width: 22, height: 22, border: "3px solid rgba(255,255,255,0.3)", borderTop: "3px solid white", borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite", flexShrink: 0 }} />
          Opening share…
        </div>
      )}
    </>
...
function Sales({ products, setProducts, customers, sales, setSales, showToast, profile }) {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState(null); // sale to delete
  ...
  const saveEditSale = () => {
    ...
    // ── Adjust stock for qty changes ──
    // For each item, compare new qty vs old qty and adjust product stock
    const oldItems = editSale.items;
    const newItems = editForm.items;

    setProducts(prev => {
      let updatedProds = [...prev];
      // Handle items that exist in old sale
      oldItems.forEach(oldItem => {
        ...
        const diff    = newQty - oldQty; // +ve = sold more (reduce stock), -ve = returned (add back to stock)
        if (diff !== 0) {
          updatedProds = updatedProds.map(p =>
            String(p.id) === String(oldItem.productId)
              ? { ...p, stock: Math.max(0, p.stock - diff) }
              : p
          );
        }
      });
      // Handle new items added during edit (not in original sale)
      newItems.forEach(newItem => {
        const wasInOld = oldItems.find(i => i.productId === newItem.productId);
        if (!wasInOld) {
          updatedProds = updatedProds.map(p =>
            String(p.id) === String(newItem.productId)
              ? { ...p, stock: Math.max(0, p.stock - newItem.qty) }
              : p
          );
        }
      });
    ...
    setEditSale(null);
    setEditForm(null);
    showToast("✅ Sale updated! Stock adjusted.");
  };
  ...
  const doDeleteSale = (s, restoreStock) => {
    // ── Restore stock if requested ──
    if (restoreStock && s.items && s.items.length > 0) {
      setProducts(prev => prev.map(p => {
        const soldItem = s.items.find(i => String(i.productId) === String(p.id));
        if (!soldItem) return p;
        return { ...p, stock: p.stock + soldItem.qty };
      }));
    }
    // ── Remove from sales (setSales will delete from Supabase automatically) ──
    setSales(prev => prev.filter(x => String(x.id) !== String(s.id)));
    setDeleteConfirm(null);
    showToast("🗑️ Sale deleted" + (restoreStock ? " · ✅ Stock restored" : ""));
  };
  ...
  if (view === "invoice" && viewInvoice) {
    ...
    return (
      <div>
        ...
        {/* ── SEND PDF via WhatsApp — ONE TAP ── */}
        <SendPdfButton viewInvoice={viewInvoice} invCust={invCust} phone={phone} profile={profile} showToast={showToast} />

        <button className="btn btn-ghost btn-full" style={{ marginTop: 8, padding: "13px", fontSize: 14 }} onClick={() => setView("list")}>← Back to Sales</button>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {deleteConfirm && (
        <Modal open={true} onClose={() => setDeleteConfirm(null)} title="Delete Sale?">
          <div style={{ marginBottom: 16 }}>
            {/* Sale summary */}
            <div style={{ background: T.roseLight, borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: T.rose, marginBottom: 4 }}>⚠️ {deleteConfirm.customerName}</div>
              <div style={{ fontSize: 12, color: T.textMid }}>{deleteConfirm.date} · {fmt(deleteConfirm.total)} · {deleteConfirm.items.length} item(s)</div>
              <div style={{ fontSize: 11, color: T.textLight, marginTop: 4 }}>
                {deleteConfirm.items.map(i => `${i.name} ×${i.qty}`).join(", ")}
              </div>
            </div>
            <div style={{ fontSize: 13, color: T.text, marginBottom: 14 }}>
              Do you also want to <b>restore the stock</b> for items in this sale?
            </div>
            {/* Stock restore info */}
            <div style={{ background: T.bg, borderRadius: 10, padding: "10px 12px", marginBottom: 4 }}>
              <div style={{ fontSize: 12, color: T.textMid, marginBottom: 6, fontWeight: 600 }}>Stock that will be restored:</div>
              {deleteConfirm.items.map((i, idx) => (
                <div key={idx} style={{ fontSize: 12, color: T.text, padding: "2px 0", display: "flex", justifyContent: "space-between" }}>
                  <span>{i.name}</span>
                  <span style={{ color: T.emerald, fontWeight: 600 }}>+{i.qty} units</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <button onClick={() => doDeleteSale(deleteConfirm, true)}
              style={{ width: "100%", padding: "13px", background: T.emerald, color: "white", border: "none", borderRadius: 12, fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
              🗑️ Delete + Restore Stock
            </button>
            <button onClick={() => doDeleteSale(deleteConfirm, false)}
              style={{ width: "100%", padding: "12px", background: T.roseLight, color: T.rose, border: `1px solid ${T.rose}30`, borderRadius: 12, fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
              🗑️ Delete Only (Keep Stock as is)
            </button>
            <button onClick={() => setDeleteConfirm(null)}
              style={{ width: "100%", padding: "11px", background: "none", color: T.textMid, border: `1px solid ${T.border}`, borderRadius: 12, fontSize: 13, cursor: "pointer" }}>
              Cancel
            </button>
          </div>
        </Modal>
...
function ReferEarn({ user, showToast }) {
  ...
  // Generate referral code: mix of letters + numbers, lowercase, e.g. "may4x9b2"
  const myCode = (() => {
    const base = (user?.email?.split("@")[0] || "user").toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 4);
    const uid4  = (user?.id || "").replace(/-/g, "").slice(0, 4).toLowerCase();
    const code  = (base + uid4).slice(0, 8).padEnd(8, "0");
    return code;
  })();
  ...
  // Check for confirm_referral in URL (admin clicked confirmation link)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const confirmId = params.get("confirm_referral");
    if (confirmId) {
      getSB().from("referrals").update({ status: "confirmed", amount: 100 }).eq("id", confirmId)
        .then(({ error }) => {
          if (!error) {
            showToast("✅ Referral confirmed successfully!");
            // Clean URL
            window.history.replaceState({}, "", window.location.pathname);
            // Reload referrals list
            getSB().from("referrals").select("*").order("created_at", { ascending: false })
  ...
  const handleSubmitReferral = async () => {
    ...
    setSubmitting(true);
    try {
      const { data: inserted, error } = await getSB().from("referrals").insert({
        referred_by:    user.id,
      ...
      showToast("✅ Referral submitted! Sending notification…");
      setName(""); setPhone("");
      // Send email to admin
      if (inserted?.id) {
        await sendReferralEmail(inserted.id, user.email, name.trim(), phone.trim());
      }
      // Reload list
...
// ─── PROFILE ─────────────────────────────────────────────────────────────────
// ─── EMAIL BACKUP BUTTON ─────────────────────────────────────────────────────
function EmailBackupButton({ products, customers, sales, userProfile, user, showToast }) {
  ...
  const sendBackup = async () => {
    if (!email.trim() || !email.includes("@")) return showToast("Enter a valid email");
    setSending(true);

    // Build backup data
    const backup = { products, customers, sales, exportedAt: new Date().toISOString(), user: user?.email };
    ...
    // Method: open mailto with body text summary + download file
    // (browsers can't send file attachments via mailto, so we download + open email)
    const body = `VManager Data Backup — ${date}

Hi,

Your VManager data backup is ready. Please find the details below.

SUMMARY:
• Products: ${products.length} (Total stock: ${totalStock} units)
• Customers: ${totalCusts}
• Sales: ${totalSales}
• Exported: ${date}

HOW TO SAVE:
1. The backup JSON file has been downloaded to your phone/device
2. Attach that file to this email and send it to yourself
3. Store it in Google Drive or any cloud storage

Backup file name: vmanager_backup_${new Date().toISOString().slice(0,10)}.json

This backup contains all your products (with stock), customers, and sales data.

Powered by VManager — Vestige Business App`;
    ...
    // Open email compose
    setTimeout(() => {
      const subject = encodeURIComponent("VManager Backup — " + date);
      const bodyEnc = encodeURIComponent(body);
      const mailUrl = `mailto:${email.trim()}?subject=${subject}&body=${bodyEnc}`;
      window.open(mailUrl, "_blank");
      setSending(false);
      setSent(true);
      showToast("✅ Email app opened! Attach the downloaded file.");
    }, 800);
  };
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }}
        onClick={() => { setShowModal(true); setSent(false); setEmail(userProfile?.email || user?.email || ""); }}>
        <div style={{ width: 40, height: 40, background: "#FFF3D6", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>📧</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 600, fontSize: 13 }}>Email Backup</div>
          <div style={{ fontSize: 11, color: T.textLight }}>Send backup to your email address</div>
        </div>
      ...
      {/* Modal */}
      {showModal && (
        <Modal open={true} onClose={() => { setShowModal(false); setSent(false); }} title="Email Backup">
          {!sent ? (
            <>
              <div style={{ marginBottom: 16 }}>
                <div style={{ background: T.amberLight, borderRadius: 12, padding: "12px 14px", marginBottom: 14 }}>
                  <div style={{ fontSize: 12, color: T.amberDark, fontWeight: 600, marginBottom: 6 }}>ℹ️ How it works:</div>
                  {[
                    "Backup JSON file downloads to your device",
                    "Your email app opens with a pre-written message",
                    "Attach the downloaded file and send",
                    "Save to Google Drive for safe recovery"
                  ].map(s => <div key={s} style={{ fontSize: 12, color: T.amberDark, padding: "2px 0" }}>• {s}</div>)}
                </div>

                <div style={{ background: T.bg, borderRadius: 12, padding: "10px 14px", marginBottom: 14 }}>
                  <div style={{ fontSize: 11, color: T.textMid, marginBottom: 4, fontWeight: 600 }}>Backup includes:</div>
                  <div style={{ fontSize: 12, color: T.text }}>📦 {products.length} products · 👥 {customers.length} customers · 🧾 {sales.length} sales</div>
                  <div style={{ fontSize: 11, color: T.textLight, marginTop: 4 }}>Total stock units: {products.reduce((a,p) => a+(p.stock||0), 0)}</div>
                </div>

                <label style={{ fontSize: 11, fontWeight: 700, color: T.textMid, textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 6 }}>Your Email Address</label>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button className="btn btn-primary" style={{ flex: 1, opacity: sending ? 0.7 : 1 }}
                  disabled={sending} onClick={sendBackup}>
                  {sending ? "⏳ Preparing…" : "📧 Send Backup"}
                </button>
              </div>
            </>
          ) : (
            <div style={{ textAlign: "center", padding: "10px 0" }}>
              ...
              <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: 16, marginBottom: 8 }}>Backup Ready!</div>
              <div style={{ fontSize: 13, color: T.textMid, lineHeight: 1.6, marginBottom: 16 }}>
                1. The JSON file was downloaded<br/>
                2. Your email app should be open<br/>
                3. Attach the file and press Send
              </div>
              <div style={{ background: T.amberLight, borderRadius: 10, padding: "10px 12px", fontSize: 12, color: T.amberDark, marginBottom: 16, textAlign: "left" }}>
                💡 <b>Tip:</b> Save the backup file to Google Drive for easy recovery anytime!
              </div>
              <button className="btn btn-primary btn-full" onClick={() => { setShowModal(false); setSent(false); }}>Done ✓</button>
            </div>
          )}
        </Modal>
      )}
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }}
          onClick={() => {
            try {
              ...
              // ── Sheet 1: Products ──
              const prodRows = [
                ["Code", "Product Name", "Category", "Buy Price (DP)", "MRP", "Stock", "Unit"],
                ...products.map(p => [p.code || "", p.name, p.category, p.buyPrice, p.mrp, p.stock, p.unit])
              ];
              const ws1 = XLSX.utils.aoa_to_sheet(prodRows);
              ws1["!cols"] = [{ wch: 10 }, { wch: 45 }, { wch: 18 }, { wch: 14 }, { wch: 10 }, { wch: 8 }, { wch: 10 }];
              XLSX.utils.book_append_sheet(wb, ws1, "Products");

              // ── Sheet 2: Sales ──
              const saleRows = [
                ["Date", "Customer", "Items", "Qty Total", "Total (₹)", "Payment Method", "Status"],
                ...sales.map(s => [
                  s.date,
                  s.customerName,
              ...
              const ws2 = XLSX.utils.aoa_to_sheet(saleRows);
              ws2["!cols"] = [{ wch: 12 }, { wch: 20 }, { wch: 50 }, { wch: 10 }, { wch: 12 }, { wch: 16 }, { wch: 10 }];
              ...
              ws3["!cols"] = [{ wch: 20 }, { wch: 14 }, { wch: 25 }, { wch: 30 }, { wch: 30 }];
              XLSX.utils.book_append_sheet(wb, ws3, "Customers");

              // ── Sheet 4: Summary ──
              const totalSales   = sales.reduce((a, s) => a + s.total, 0);
              const paidSales    = sales.filter(s => s.paymentStatus === "paid").reduce((a, s) => a + s.total, 0);
              const pendingSales = totalSales - paidSales;
              const summaryRows = [
                ["VManager Backup Summary", ""],
                ...
                ["Paid Amount (₹)", paidSales],
                ["Pending Amount (₹)", pendingSales],
              ];
              ...
              // Download
              const fname = "vmanager_backup_" + new Date().toISOString().slice(0, 10) + ".xlsx";
          ...
          <div style={{ width: 40, height: 40, background: "#E6FBF5", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>📊</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 13 }}>Backup Data (Excel)</div>
            <div style={{ fontSize: 11, color: T.textLight }}>Download as .xlsx — Products, Sales, Customers</div>
          </div>
          <span style={{ color: T.textLight, fontSize: 16 }}>⬇️</span>
        </div>

        {/* Export CSV — Products only */}
        <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }}
          onClick={() => {
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 13 }}>Export Products (CSV)</div>
            <div style={{ fontSize: 11, color: T.textLight }}>Download products as CSV</div>
          </div>
          <span style={{ color: T.textLight, fontSize: 16 }}>⬇️</span>
        </div>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }}
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 13 }}>Export Sales (CSV)</div>
            <div style={{ fontSize: 11, color: T.textLight }}>Download sales report as CSV</div>
          </div>
          <span style={{ color: T.textLight, fontSize: 16 }}>⬇️</span>
        </div>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }}
          onClick={async () => {
            showToast("⏳ Syncing all data to cloud…");
            const uid2 = user?.id;
            if (!uid2) return showToast("❌ Not logged in");
            try {
              // Force sync ALL products with stock to Supabase
              let synced = 0;
              for (const p of products) {
                await sbUpsertRow("products", {
                  id: String(p.id), user_id: uid2,
              ...
              showToast("✅ " + synced + " products synced to cloud!");
            } catch(e) {
              showToast("❌ Sync failed: " + e.message);
            }
          }}>
          <div style={{ width: 40, height: 40, background: "#EEF2FF", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>☁️</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 13 }}>Force Sync to Cloud</div>
            <div style={{ fontSize: 11, color: T.textLight }}>Save all stock & data to Supabase now</div>
          </div>
          <span style={{ color: T.textLight, fontSize: 16 }}>🔄</span>
        </div>

        {/* Email Backup */}
        <EmailBackupButton products={products} customers={customers} sales={sales} userProfile={userProfile} user={user} showToast={showToast} />
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 0", cursor: "pointer" }}
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: T.textLight }}>Import from Excel backup (.xlsx)</div>
          </div>
          <span style={{ color: T.textLight, fontSize: 16 }}>⬆️</span>
        </div>
        <input id="restore-input" type="file" accept=".xlsx,.xls" style={{ display: "none" }}
          onChange={e => {
            ...
            reader.onload = evt => {
              try {
                ...
                // ── Restore Products ──
                const ws1 = wb.Sheets["Products"];
                if (ws1) {
                  const rows = XLSX.utils.sheet_to_json(ws1, { header: 1 }).slice(1); // skip header
                  const restoredProducts = rows
                    .filter(r => r[1]) // must have name
                    .map((r, i) => ({
                      id:        genUid(),
                      code:      r[0] || "",
                      name:      r[1] || "",
                  ...
                  if (restoredProducts.length > 0) setProducts(restoredProducts);
                }

                // ── Restore Customers ──
                const ws3 = wb.Sheets["Customers"];
                if (ws3) {
                  const rows = XLSX.utils.sheet_to_json(ws3, { header: 1 }).slice(1);
                  const restoredCustomers = rows
                    .filter(r => r[0])
                    .map(r => ({
                      id:      genUid(),
                      name:    r[0] || "",
                ...
                showToast("✅ Data restored from Excel!");
              } catch (err) {
          ...
          onChange={e => {
            ...
            reader.onload = evt => {
              try {
                ...
                if (data.products) {
                  // Restore products with stock — force sync all to Supabase
                  setProducts(data.products);
                  // Also save to localStorage immediately
                  const uid2 = data.products[0]?.user_id || userIdRef?.current;
                  if (uid2) {
                ...
                showToast("✅ Data restored! Syncing to cloud…");
      ...
      {/* ── INSTALL APP CARD ── */}
      <InstallAppCard showToast={showToast} />
...
// ── Email notification for referrals ──────────────────────────────────────
const ADMIN_EMAIL = "myur123patel@gmail.com";

async function sendReferralEmail(referralId, referrerEmail, referredName, referredPhone) {
  try {
    const confirmUrl = window.location.origin + "?confirm_referral=" + referralId;
    const subject = encodeURIComponent("New VManager Referral — " + referredName);
    const body = encodeURIComponent(
      "New referral submitted on VManager!\n\n" +
      "Referred By: " + referrerEmail + "\n" +
      "Referred Name: " + referredName + "\n" +
      "Referred Phone: " + referredPhone + "\n\n" +
      "To CONFIRM this referral, click this link:\n" +
      confirmUrl + "\n\n" +
      "— VManager Notifications"
    );
...
// Save entire products/customers/sales array as JSON blob in user_data table
async function sbSaveUserData(uid, key, value) {
  const json = JSON.stringify(value);
  const { error } = await getSB().from("user_data").upsert(
    { user_id: uid, key, value: json, updated_at: new Date().toISOString() },
    { onConflict: "user_id,key" }
  );
  if (error) {
    // Fallback: try without onConflict
    const { error: e2 } = await getSB().from("user_data").upsert({ user_id: uid, key, value: json, updated_at: new Date().toISOString() });
    if (e2) console.error("sbSaveUserData FAILED:", e2.message);
  }
}

async function sbLoadUserData(uid, key) {
  const { data, error } = await getSB().from("user_data").select("value").eq("user_id", uid).eq("key", key).single();
  if (error) return null;
  try { return JSON.parse(data.value); } catch { return null; }
}

// Still used for customers and sales
...
// ─── INSTALL APP CARD (shown in Profile) ─────────────────────────────────────
function InstallAppCard({ showToast }) {
  const [showPrompt, setShowPrompt] = useState(false);
  const [step, setStep]             = useState("main"); // main | ios
  const isIOS     = /iphone|ipad|ipod/i.test(navigator.userAgent);
  const isSafari  = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
  const isAndroid = /android/i.test(navigator.userAgent);
  const canNative = !!window.__installPrompt;
  ...
  const handleInstall = async () => {
    if (canNative) {
      ...
      if (outcome === "accepted") showToast("✅ VManager added to Home Screen!");
      else showToast("You can install anytime from here.");
  ...
  return (
    <>
      {/* Card in Profile */}
      <div className="card" style={{ marginBottom: 12, background: `linear-gradient(135deg, ${T.navy} 0%, #1a2a6c 100%)`, border: "none" }}>
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          {/* App icon */}
          <div style={{ width: 54, height: 54, borderRadius: 14, background: T.amber, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: `0 4px 16px ${T.amber}60` }}>
            <span style={{ fontFamily: "Syne,sans-serif", fontWeight: 900, fontSize: 26, color: T.navy }}>V</span>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 15, color: "white" }}>
              Add to Home Screen
            </div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", marginTop: 3, lineHeight: 1.5 }}>
              Use VManager like a native app — no browser needed
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          {[
            ...
            <div key={txt} style={{ background: "rgba(255,255,255,0.08)", borderRadius: 10, padding: "8px 10px", display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontSize: 16 }}>{ic}</span>
              <span style={{ fontSize: 12, color: "rgba(255,255,255,0.75)", fontWeight: 500 }}>{txt}</span>
            </div>
          ))}
        </div>
        ...
        <button
          ...
          style={{ width: "100%", marginTop: 14, padding: "13px", background: T.amber, color: T.navy, border: "none", borderRadius: 12, fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: `0 6px 20px ${T.amber}50` }}>
          📲 Add VManager to Home Screen
        </button>
      </div>

      {/* iOS step-by-step popup */}
      {showPrompt && step === "ios" && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 9999, display: "flex", alignItems: "flex-end", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "white", borderRadius: 24, padding: 28, width: "100%", maxWidth: 420, boxShadow: "0 -20px 60px rgba(0,0,0,0.3)" }}>
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              <div style={{ width: 64, height: 64, borderRadius: 18, background: T.navy, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", boxShadow: `0 4px 16px ${T.navy}40` }}>
                <span style={{ fontFamily: "Syne,sans-serif", fontWeight: 900, fontSize: 30, color: T.amber }}>V</span>
              </div>
              <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 20, color: T.navy }}>Add to Home Screen</div>
              <div style={{ fontSize: 13, color: T.textMid, marginTop: 6 }}>Follow these steps in Safari browser</div>
            </div>

            {[
              ["1️⃣", "Open in Safari", "This app must be opened in Safari (not Chrome) to install"],
              ["2️⃣", "Tap Share button 📤", "Find the Share icon at the bottom of the Safari browser"],
              ["3️⃣", "Tap \"Add to Home Screen\"", "Scroll down in the Share menu to find this option"],
              ["4️⃣", "Tap \"Add\"", "VManager icon will appear on your iPhone home screen!"],
            ].map(([num, title, sub]) => (
              <div key={num} style={{ display: "flex", gap: 14, alignItems: "flex-start", padding: "10px 0", borderBottom: `1px solid ${T.border}` }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: T.textMid, marginTop: 2, lineHeight: 1.5 }}>{sub}</div>
                </div>
              </div>
            ))}

            <div style={{ marginTop: 14, background: T.amberLight, borderRadius: 12, padding: "12px 14px", display: "flex", gap: 10, alignItems: "flex-start" }}>
              <span style={{ fontSize: 20 }}>💡</span>
              <div style={{ fontSize: 12, color: T.amberDark, lineHeight: 1.5 }}>
                <b>Android users:</b> In Chrome tap the 3-dot menu ⋮ → "Add to Home screen"
              </div>
            </div>
...
// ─── INSTALL PROMPT (shown after registration) ────────────────────────────────
...
function ResetPasswordScreen({ onDone }) {
  ...
  const goToLogin = () => {
    window.location.href = "https://vmanager-nine.vercel.app";
  };
    ...
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg, ${T.navy}, #1a2a6c)`, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ width: "100%", maxWidth: 380 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 900, fontSize: 36, color: T.amber }}>
            V<span style={{ color: "white" }}>Manager</span>
          </div>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginTop: 6 }}>Vestige Business App</div>
        </div>

        {success ? (
          /* Success state */
          <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 24, padding: 32, textAlign: "center" }}>
            <div style={{ fontSize: 72, marginBottom: 16, animation: "popIn 0.5s ease both" }}>✅</div>
            <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 900, fontSize: 24, color: T.amber, marginBottom: 10 }}>
              Password Updated!
            </div>
            <div style={{ fontSize: 14, color: "rgba(255,255,255,0.65)", lineHeight: 1.7, marginBottom: 28 }}>
              Your password has been changed successfully. You can now sign in with your new password.
            </div>
            <button onClick={goToLogin}
              style={{ width: "100%", padding: "15px", background: T.amber, color: T.navy, border: "none", borderRadius: 14, fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 16, cursor: "pointer", boxShadow: `0 8px 24px ${T.amber}50` }}>
              👋 Go to Login →
            </button>
          </div>
        ) : (
          /* Form */
          <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 24, padding: 28 }}>
            <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 22, color: "white", marginBottom: 6 }}>
              🔐 Set New Password
            </div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginBottom: 24 }}>
              Enter a new password for your VManager account.
            </div>

            {err && (
              <div style={{ background: "rgba(240,71,112,0.15)", border: "1px solid rgba(240,71,112,0.4)", color: "#ff7c96", padding: "12px 14px", borderRadius: 12, fontSize: 13, fontWeight: 600, marginBottom: 16 }}>
                ⚠️ {err}
              </div>
            )}
            ...
            {newPass.length > 0 && (
              <div style={{ marginBottom: 16, display: "flex", gap: 6, alignItems: "center" }}>
                {[1,2,3,4].map(n => (
                  <div key={n} style={{ flex: 1, height: 4, borderRadius: 2, background: newPass.length >= n * 2 ? (newPass.length >= 8 ? T.emerald : T.amber) : "rgba(255,255,255,0.15)" }} />
                ))}
                <span style={{ fontSize: 11, color: newPass.length >= 8 ? T.emerald : T.amber, marginLeft: 6, whiteSpace: "nowrap" }}>
                  {newPass.length < 6 ? "Too short" : newPass.length < 8 ? "OK" : "Strong ✓"}
                </span>
              </div>
            )}

            <button onClick={handleReset} disabled={loading}
              style={{ width: "100%", padding: "14px", background: loading ? "#999" : T.amber, color: T.navy, border: "none", borderRadius: 14, fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, boxShadow: loading ? "none" : `0 8px 24px ${T.amber}50` }}>
              {loading
                ? <><span style={{ width: 18, height: 18, border: `3px solid ${T.navy}30`, borderTop: `3px solid ${T.navy}`, borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite" }} /><span>Updating…</span></>
                : "🔐 Update Password"
              }
            </button>

            <button onClick={onDone}
              style={{ width: "100%", marginTop: 10, padding: "10px", background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: 13, cursor: "pointer" }}>
              ← Back to Login
            </button>
          </div>
        )}
...
function AuthScreen({ onLogin }) {
  // Detect Google OAuth new user redirect
  const googleSignup = (() => {
    try {
      const params = new URLSearchParams(window.location.search);
      if (params.get("google_plan") === "1") {
        const data = JSON.parse(localStorage.getItem("vm_google_signup") || "{}");
  ...
  const [screen,  setScreen]  = useState(googleSignup ? "plan" : "login"); // login | register | plan | paying | success | forgot
  ...
  // Register
  const [rName,   setRName]   = useState(googleSignup?.name  || "");
  const [rPhone,  setRPhone]  = useState(googleSignup?.phone || "");
  const [rEmail,  setREmail]  = useState(googleSignup?.email || "");
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // Supabase redirects to Google — page will reload with session
    } catch (e) {
  ...
  const handlePayment = async () => {
    ...
    const createAccount = async (paymentId) => {
      ...
      try {
        ...
        if (googleSignup?.uid) {
          // Google user — already authenticated, just get their session
          const { data: { session } } = await getSB().auth.getSession();
          sessionUser = session?.user || null;
          if (!sessionUser) throw new Error("Google session expired. Please sign in again.");
          // Clear the google_signup flag
          localStorage.removeItem("vm_google_signup");
          window.history.replaceState({}, "", window.location.pathname);
        } else {
          // Email/password user — sign up normally
          let supaUser;
          ...
          try {
            const { data: sd } = await getSB().auth.signInWithPassword({ email: rEmail.trim(), password: rPass });
            if (sd?.user) supaUser = sd.user;
          } catch {}
          await new Promise(r => setTimeout(r, 300));
          sessionUser = supaUser;
        }
  ...
  const handleForgot = async () => {
    ...
    try {
      ...
      // Supabase returns success even if email not found (security)
      // But show real errors like network issues
      if (e.message?.includes("rate limit") || e.message?.includes("too many")) {
        ...
        // For most errors, still show success (don't reveal if email exists)
        setFSent(true);
      }
    }
    finally { setLoading(false); }
      ...
      <div style={{ width: "100%", maxWidth: 390 }}>
        ...
        <div style={{ background: "white", borderRadius: 24, padding: 24, boxShadow: "0 24px 60px rgba(0,0,0,0.4)" }}>
          {fSent ? (
            <div style={{ textAlign: "center", padding: "10px 0" }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>📧</div>
              <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: 16, color: T.navy, marginBottom: 8 }}>Email Sent!</div>
              <div style={{ fontSize: 13, color: T.textMid, lineHeight: 1.6, marginBottom: 16 }}>
                We sent a password reset link to <b>{fEmail}</b>.
              </div>
              {/* Step by step instructions */}
              {[
                ["1️⃣", "Open your email app"],
                ["2️⃣", "Look for email from VManager / Supabase"],
                ["3️⃣", "Check Spam / Junk folder too"],
                ["4️⃣", "Click the reset link in the email"],
                ["5️⃣", "Set your new password"],
              ].map(([num, text]) => (
                <div key={num} style={{ display: "flex", gap: 10, alignItems: "center", padding: "6px 0", borderBottom: `1px solid ${T.border}`, textAlign: "left" }}>
                  <span style={{ fontSize: 16, flexShrink: 0 }}>{num}</span>
                  <span style={{ fontSize: 13, color: T.text }}>{text}</span>
                </div>
              ))}
              <div style={{ background: T.amberLight, borderRadius: 12, padding: "12px 14px", fontSize: 12, color: T.amberDark, marginTop: 14, marginBottom: 16, textAlign: "left" }}>
                ⚠️ <b>Not received?</b> This usually means SMTP email is not set up in Supabase. Contact the app admin.
              </div>
      ...
      <div style={{ width: "100%", maxWidth: 390, background: "white", borderRadius: 28, overflow: "hidden", boxShadow: "0 32px 80px rgba(0,0,0,0.5)" }}>
        ...
        <div style={{ padding: "0 24px 28px" }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input className="input" style={{ paddingLeft: 40, textTransform: "uppercase", letterSpacing: 2 }}
                  value={rCode} onChange={e => setRCode(e.target.value.toLowerCase().replace(/[^a-z0-9]/g, ""))} placeholder="Enter referral code (e.g. may4x9b2)" />
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "16px 0 12px" }}>
            ...
            <span style={{ fontSize: 11, color: T.textLight, fontWeight: 600, whiteSpace: "nowrap" }}>OR CONTINUE WITH</span>
            <div style={{ flex: 1, height: 1, background: T.border }} />
          </div>

          {/* Google Sign In button */}
          <button onClick={handleGoogleLogin} disabled={loading}
            style={{ width: "100%", padding: "13px 16px", background: "white", color: "#3c4043", border: `1.5px solid #dadce0`, borderRadius: 14, fontFamily: "DM Sans,sans-serif", fontWeight: 600, fontSize: 14, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, boxShadow: "0 2px 8px rgba(0,0,0,0.08)", transition: "all 0.2s", marginBottom: 4 }}
          ...
          {/* Switch tab */}
          <div style={{ textAlign: "center", marginTop: 14, fontSize: 13, color: T.textMid }}>
...
export default function App() {
  ...
  const [resetMode, setResetMode]   = useState(false); // true when user clicked reset link in email
  ...
  const loadUserData = useCallback(async (uid) => {
    ...
    try {
      ...
      // Load products from user_data blob (most reliable)
      const savedProds = await sbLoadUserData(uid, "products");
      if (savedProds && savedProds.length > 0) {
        setProductsRaw(savedProds);
      } else if (prods.length > 0) {
        // Fallback: load from products table (old format)
        const mapped = prods.map(r => ({ ...r, buyPrice: r.buy_price ?? r.buyPrice ?? 0 }));
        setProductsRaw(mapped);
        // Migrate to new blob format
        sbSaveUserData(uid, "products", mapped);
      } else {
        // First ever login — save seedProducts to blob
        sbSaveUserData(uid, "products", seedProducts);
      }
  ...
  useEffect(() => {
    ...
    const { data: { subscription: authSub } } = sb.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "SIGNED_IN" && session?.user) {
        ...
        // Check if user has a profile already (existing user)
        const existingPr = await sbGetProfile(googleUser.id);
        if (existingPr?.paid_at) {
          // Existing user — log them in directly
          const sub = checkSubscription(getBestProfile(googleUser.id, existingPr));
          setSubStatus(sub.status);
          if (sub.active) {
            userIdRef.current = googleUser.id;
            setUser(googleUser);
            setUserProfile(existingPr);
            if (existingPr.name) setProfile(prev => ({ ...prev, name: existingPr.name, phone: existingPr.phone || "" }));
            setDataReady(false);
          ...
          // New Google user — show plan selection
          // Pre-fill their name and email from Google
          setAuthReady(true);
          // We store Google user info in localStorage for the plan screen to use
          localStorage.setItem("vm_google_signup", JSON.stringify({
            uid:   googleUser.id,
            name:  meta.full_name || meta.name || "",
            email: googleUser.email || "",
            phone: "",
          }));
          window.location.replace(window.location.origin + "?google_plan=1");
        }
        return;
      }
      if (event === "SIGNED_OUT") {
    ...
    // Method 2: Hash fallback — check URL hash for type=recovery
  ...
  const setProducts = useCallback((updater) => {
    setProductsRaw(prev => {
      ...
      if (currentUid) {
        // Save entire product list as a single blob — most reliable approach
        sbSaveUserData(currentUid, "products", next);
      }
      return next;
    });
  }, [uid]);
  ...
  const setSales = useCallback((updater) => {
    setSalesRaw(prev => {
      ...
      if (uid) {
        ...
        // ── Delete removed sales from Supabase ──
        const nextIds = new Set(next.map(s => String(s.id)));
        prev.filter(s => !nextIds.has(String(s.id))).forEach(s => {
          sbDeleteRow("sales", s.id);
        });
      }
      return next;
    });
  ...
  if (resetMode) return (
    <>
      <style>{css}</style>
      <ResetPasswordScreen onDone={() => { window.location.replace(window.location.origin); }} />
    </>
  );
