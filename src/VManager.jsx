...
function SendPdfButton({ viewInvoice, invCust, phone, profile, showToast }) {
  ...
  const loadLibs = () => new Promise((resolve, reject) => {
    ...
    const err  = () => reject(new Error("Failed to load PDF library"));
  ...
  const buildInvoiceEl = () => {
    const invNum   = String(viewInvoice.id).slice(-6).toUpperCase();
    const isPaid   = viewInvoice.paymentStatus === "paid";
    const distName = profile?.name || ""; const distPhone = profile?.phone || "";
    const distId   = profile?.vestId || ""; const distAddr = profile?.address || "";

    const el = document.createElement("div");
    el.style.cssText = "width:420px;background:white;font-family:Arial,sans-serif;font-size:13px;position:fixed;left:-9999px;top:0;z-index:-1;";
    el.innerHTML = `
      ...
      <div style="padding:18px 22px;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px;margin-bottom:14px;">
          <thead><tr style="border-bottom:2px solid #0B1437;">
            ...
            <th style="text-align:center;padding:5px 0;font-size:9px;color:#8896A5;font-weight:700;text-transform:uppercase;width:40px;">Qty</th>
            <th style="text-align:right;padding:5px 0;font-size:9px;color:#8896A5;font-weight:700;text-transform:uppercase;width:80px;">Amount</th>
          </tr></thead>
          <tbody>
            ${viewInvoice.items.map(i => `<tr style="border-bottom:1px solid #f5f5f5;">
      ...
      <div style="background:#f9f9f9;padding:14px 22px;border-top:1px solid #eee;">
        ...
        <div style="font-size:11px;color:#333;line-height:1.8;">
          ${distName ? "👤 " + distName + "<br/>" : ""}
          ${distPhone ? "📞 " + distPhone + "<br/>" : ""}
          ${distId ? "🆔 Vestige ID: " + distId + "<br/>" : ""}
          ${distAddr ? "📍 " + distAddr : ""}
        </div>
        <div style="text-align:center;margin-top:12px;font-size:11px;color:#8896A5;">Thank you for your purchase! 🙏<br/><span style="font-size:9px;color:#bbb;">Powered by VManager</span></div>
      </div>`;
  ...
  const handleSendPdf = async () => {
    ...
    try {
      ...
      const invNum  = String(viewInvoice.id).slice(-6).toUpperCase();
      const fileName = "Invoice_" + invNum + "_" + viewInvoice.customerName.replace(/\s+/g, "_") + ".pdf";
      const pdfBlob = pdf.output("blob");
      const pdfFile = new File([pdfBlob], fileName, { type: "application/pdf" });

      // Try Web Share API (one-tap share to WhatsApp on mobile)
      if (navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
        await navigator.share({
          files:  [pdfFile],
          title:  "Invoice #" + invNum,
          text:   "Invoice from " + (profile?.name || "VManager") + " — ₹" + viewInvoice.total.toLocaleString("en-IN"),
        });
        ...
        showToast("📄 PDF downloaded! Now share it on WhatsApp.");
      }
    } catch (e) {
      if (e.name !== "AbortError") {
        showToast("❌ " + (e.message || "PDF failed. Try again."));
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      <button
        ...
        style={{ marginTop: 8, padding: "16px", fontSize: 15, fontWeight: 800, background: T.navy, color: "white", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, opacity: loading ? 0.75 : 1 }}
        disabled={loading}
        onClick={handleSendPdf}
      >
        {loading
          ? <><span style={{ width: 20, height: 20, border: "3px solid rgba(255,255,255,0.2)", borderTop: "3px solid white", borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite" }} /><span>Generating PDF…</span></>
          : <><span style={{ fontSize: 20 }}>📤</span><span>Send PDF to {phone ? (invCust?.name?.split(" ")[0] || "Customer") + " · " + phone : "Customer"}</span></>
        }
      </button>
    </>
  );
