...
export default function App() {
  ...
  const loadUserData = useCallback(async (uid) => {
    ...
    try {
      ...
      // If no products in Supabase yet → push all seedProducts to Supabase now
      // so future refreshes load them correctly
      if (prods.length === 0) {
        console.log("📦 First load — pushing seedProducts to Supabase");
        seedProducts.forEach(p => sbUpsertRow("products", {
          id: String(p.id), user_id: uid,
          name: p.name, category: p.category,
          buy_price: p.buyPrice, mrp: p.mrp ?? 0,
          stock: p.stock ?? 0, unit: p.unit,
          code: p.code || "",
        }));
        // Keep seedProducts in state (already there from useState)
      }
