  ...
  const loadUserData = useCallback(async (uid) => {
    ...
    try {
      ...
      if (prods.length > 0) {
        ...
        try { localStorage.setItem("vm_products_" + uid, JSON.stringify(mapped)); } catch {}
      } else {
        // Supabase has no products — check localStorage cache first
        try {
          const cached = localStorage.getItem("vm_products_" + uid);
          if (cached) {
            const cachedProds = JSON.parse(cached);
            if (cachedProds && cachedProds.length > 0) {
              setProductsRaw(cachedProds);
              console.log("📦 Products loaded from localStorage cache:", cachedProds.length);
              // Re-sync all cached products to Supabase in background
              cachedProds.forEach(p => sbUpsertRow("products", {
  ...
  const setProducts = useCallback((updater) => {
    setProductsRaw(prev => {
      ...
      try {
        ...
        if (currentUid) {
          localStorage.setItem("vm_products_" + currentUid, JSON.stringify(next));
        }
      } catch {}
