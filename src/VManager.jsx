...
async function sendReferralEmail(referralId, referrerEmail, referredName, referredPhone) {
  // Use a simple fetch to a free email service (Formspree or EmailJS)
  // Using Supabase Edge Function approach via RPC or direct email
  try {
    // Build confirmation URL — admin clicks this to confirm referral
    const confirmUrl = window.location.origin + "?confirm_referral=" + referralId;
    
    // Use fetch to send via Formspree (free, no signup needed for basic use)
    // We'll use a mailto fallback + Supabase notification
    ...
    const body = encodeURIComponent(
      ...
      "To CONFIRM this referral and mark it as completed, click:\n" +
      confirmUrl + "\n\n" +
      "— VManager Notifications"
    );

    // Open mailto as primary (always works, no API key needed)
...
async function sbDeleteRow(table, id) {
  // Retry delete up to 3 times to ensure it sticks
  for (let i = 0; i < 3; i++) {
    const { error } = await getSB().from(table).delete().eq("id", String(id));
    if (!error) return;
    console.warn(`Delete ${table} attempt ${i+1} failed:`, error.message);
...
export default function App() {
  ...
  const loadUserData = useCallback(async (uid) => {
    ...
    try {
      ...
      // If Supabase has no products, seedProducts already in state
  ...
  const setProducts = useCallback((updater) => {
    setProductsRaw(prev => {
      ...
      // Only sync CHANGED products to Supabase (not all 176 at once)
      const currentUid = userIdRef.current;
      if (currentUid) {
        // Find products that changed (different from prev)
        const changed = next.filter(p => {
          const old = prev.find(o => String(o.id) === String(p.id));
          return !old ||
            old.stock !== p.stock || old.mrp !== p.mrp ||
            old.buyPrice !== p.buyPrice || old.name !== p.name ||
            old.category !== p.category || old.unit !== p.unit;
        });
        // Only upsert changed ones
        changed.forEach(p => sbUpsertRow("products", {
          id: String(p.id), user_id: currentUid,
          name: p.name, category: p.category,
          buy_price: p.buyPrice, mrp: p.mrp,
          stock: p.stock, unit: p.unit,
          code: p.code || "",
