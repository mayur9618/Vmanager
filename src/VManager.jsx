...
function Profile({ showToast, profile, setProfile, onLogout, userProfile, user, products, customers, sales, setProducts, setCustomers, setSales }) {
  ...
  const [freshProfile, setFreshProfile] = useState(userProfile);

  // Refresh profile data from Supabase on mount to get latest plan/paid_at
  useEffect(() => {
    if (!user) return;
    sbGetProfile(user.id).then(pr => {
      if (pr) setFreshProfile(pr);
    });
  }, [user]);
...
async function sbSaveProfile(uid, payload) {
  const { error } = await getSB().from("profiles").upsert({ id: uid, ...payload });
...
function checkSubscription(pr) {
  ...
  if (pr.paid === false) return { active: false, expired: false, daysLeft: 0, status: "blocked" };
  ...
  const handlePayment = async () => {
    ...
    const createAccount = async (paymentId) => {
      ...
      try {
        ...
        // Retry save up to 5 times with increasing delays
        let saved = false;
        const delays = [300, 600, 1000, 1500, 2000];
        for (let attempt = 0; attempt < 5; attempt++) {
          await new Promise(r => setTimeout(r, delays[attempt]));
          const { error: saveErr } = await getSB().from("profiles").upsert({
            id: sessionUser.id, ...profilePayload
          });
          if (!saveErr) { saved = true; break; }
          console.warn(`Profile save attempt ${attempt + 1} failed:`, saveErr.message);
        }
...
export default function App() {
  ...
  const handleLogin = useCallback(async (supaUser, pr) => {
    ...
    // Only re-fetch from Supabase if no profile was passed (e.g. session restore)
    let profileToUse = pr;
    if (!pr || !pr.paid_at) {
      // Check localStorage fallback first (in case Supabase write failed)
      const pending = localStorage.getItem("vm_pending_profile_" + actualUser.id);
      if (pending) {
        try {
          profileToUse = JSON.parse(pending);
          // Retry saving to Supabase in background
          getSB().from("profiles").upsert({ id: actualUser.id, ...profileToUse }).then(({ error }) => {
            if (!error) localStorage.removeItem("vm_pending_profile_" + actualUser.id);
          });
        } catch {}
      }
      if (!profileToUse || !profileToUse.paid_at) {
        // No profile passed and no fallback → fetch from Supabase (existing session login)
        try { const fp = await sbGetProfile(actualUser.id); if (fp) profileToUse = fp; } catch {}
      }
    }
