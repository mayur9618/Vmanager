      ...
      <div style={{ width: "100%", maxWidth: 380 }}>
        ...
        {success ? (
          ...
          <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 24, padding: 32, textAlign: "center" }}>
            <div style={{ fontSize: 64, marginBottom: 16, animation: "popIn 0.5s ease both" }}>✅</div>
            <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 22, color: T.amber, marginBottom: 10 }}>
              Password Updated!
            </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // ── RECOVERY DETECTION ──
    // Method 1: onAuthStateChange fires PASSWORD_RECOVERY when user clicks email link
    const { data: { subscription: authSub } } = sb.auth.onAuthStateChange((event, session) => {
      if (event === "PASSWORD_RECOVERY") {
        // User clicked reset link — show set new password screen
        setResetMode(true);
        setAuthReady(true);
        return;
      }
  ...
  if (resetMode) return (
    <>
      <style>{css}</style>
      <ResetPasswordScreen onDone={() => { setResetMode(false); window.location.hash = ""; }} />
    </>
  );
