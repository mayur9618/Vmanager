    ...
    if (localRaw) {
      try {
        ...
        if (localPr?.paid === true) {
          getSB().from("profiles").upsert({ id: actualUser.id, ...localPr }).then(({ error }) => {
            if (!error) { console.log("✅ Profile synced to Supabase"); }
            else { console.warn("Profile sync failed:", error.message); }
          });
        }
      } catch {}
    }
